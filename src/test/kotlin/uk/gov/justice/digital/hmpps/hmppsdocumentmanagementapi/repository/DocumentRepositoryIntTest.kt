...
class DocumentRepositoryIntTest : IntegrationTestBase() {
  ...
  fun `retrieve document with metadata history from database`() {
    ...
    with(document) {
      ...
      assertThat(fileHash).isEqualTo("d58e3582afa99040e27b92b13c8f2280")
      assertThat(mimeType).isEqualTo("application/pdf")
      assertThat(metadata["prisonNumbers"].single().asText()).isEqualTo("C3456DE")
      assertThat(metadata["prisonCodes"].single().asText()).isEqualTo("KMI")
      assertThat(metadata).isEqualTo(JacksonUtil.toJsonNode("{\"prisonNumbers\": [\"C3456DE\"], \"prisonCodes\": [\"KMI\"]}"))
    ...
    with(document.documentMetadataHistory()) {
      ...
      with(get(0)) {
        ...
        assertThat(metadata["prisonNumbers"].single().asText()).isEqualTo("B2345CD")
        assertThat(metadata["prisonCodes"].single().asText()).isEqualTo("KMI")
        assertThat(metadata).isEqualTo(JacksonUtil.toJsonNode("{\"prisonNumbers\": [\"B2345CD\"], \"prisonCodes\": [\"KMI\"]}"))
